...
**Consistência entre ETL e análise:** o ETL roda uma única vez, no início do notebook, sobre os dataframes globais (`prd`, `ped`, `pgpd`, `itp`, `avl`, `clt`, `vndd`). Todas as 9 perguntas de negócio (Q1–Q9) reutilizam essas mesmas variáveis já tratadas — nenhuma célula de análise recarrega os CSVs brutos do disco. Isso garante que o princípio de zero-loss valha de ponta a ponta: uma categoria de produto sem cadastro, por exemplo, não desaparece silenciosamente de um `groupby` por chegar como `NaN` numa cópia paralela dos dados.
