...
def q8_intervencao_sellers(df: pd.DataFrame):
    # 1 linha por (pedido, seller): review_score é do pedido, não do item —
    # sem isso, um seller com 3 itens numa mesma review negativa seria contado 3x
    seller_order = df.drop_duplicates(['order_id', 'seller_id'])

    reviews_neg = seller_order[seller_order['review_score'].isin([1, 2])]
    seller_reviews = reviews_neg.groupby('seller_id')['review_score'].count().reset_index(name='bad_reviews')
    
    all_reviews = seller_order.groupby('seller_id')['review_score'].mean().reset_index(name='nota_media')
...
def q9_risco_receita(df: pd.DataFrame):
    ...
    # 1 linha por (pedido, seller) — mesmo motivo do Q8: review_score é do pedido, não do item
    seller_order = df.drop_duplicates(['order_id', 'seller_id'])
    reviews_neg = seller_order[seller_order['review_score'].isin([1, 2])]
